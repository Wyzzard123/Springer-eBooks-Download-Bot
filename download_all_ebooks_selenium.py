...
def download_all_ebooks_selenium(driver, txt_file="Springer All Free Ebooks.txt", delay=2, background=True, folder=None):
    """
    Example Usage:            python download_all_ebooks_selenium.py [delay] [reference file name with quotes]
    ...
    Example:            python download_all_ebooks_selenium.py 10
    ...
    Example:            python download_all_ebooks_selenium.py 2 "Springer All Free Ebooks after failure.txt"
    ...
    :param delay:       Passed in as a command line argument. Delay in seconds. If not passed in, defaults to 2.
                        ...
                        Usage: python download_all_ebooks_selenium.py [delay/s]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if not os.path.isdir(folder):
            raise Exception("Usage: python download_all_ebooks_selenium.py [download folder] [txt_file] [delay/s] [background]\n"
    ...
    if len(sys.argv) > 3:
        if not sys.argv[3].isnumeric():
            raise ValueError("Usage: python download_all_ebooks_selenium.py [download folder] [txt_file] [delay/s] [background]\n"
    ...
    download_all_ebooks_selenium(driver=driver, folder=folder, txt_file=txt_file, delay=delay, background=background)
